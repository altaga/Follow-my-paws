...
void loop() {
  ...
  int c=30;
  if((abs(ax-axm)*c)>10 && (abs(ay-aym)*c)>10 && (abs(az-azm)*c)>10)
  {
  ...
  if(current2-mem2>2000)
  {
    mem2=current2;
    SerialUSB.println(current-mem);
  }
  current2=millis();

  if((current-mem)>60000)
  { 
  ...
  sen = string2char("{ \"ID\" : \"DOG1\",\"Number\" : \"01\",\"moisture\" : \""+String(int(humi))+"\",\"accelerometer\" : \""+String(int(accel))+"\",\"temperature\" : \""+String(int(temp))+"\",\"air\" : \"10\"}");
  sprintf(data,sen);
